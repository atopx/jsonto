...
use crate::word_case::to_singular;
use crate::word_case::string_hashmap;
...
pub fn to(name: &str, shape: &Shape, options: Options) -> Code {
...
fn generate_struct_from_field_shapes(
    ...
    for (name, typ) in map.iter() {
        let (was_optional, collapsed) = shape::collapse_option(typ);
